...
module('Unit | Model | channel', function (hooks) {
  ...
  test('#formattedTopic turns URLs into links', function (assert) {
    ...
    assert.equal(
      channel.formattedTopic.toString(),
      'visit <a href="https://kosmos.org" class="linkified" target="_blank" rel="nofollow">kosmos.org</a> for more info'
    );
  });
  ...
  test('#shortName', function (assert) {
    ...
    assert.equal(
      channel.shortName,
      'kosmos-dev',
      'returns name without hash for IRC'
    );
    ...
    assert.equal(
      channel.shortName,
      'kosmos-dev',
      'returns name without MUC domain for XMPP'
    );
  });
});
